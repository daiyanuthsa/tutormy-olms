...
const FilterButton = ({ label, filter, activeFilter, setActiveFilter }) => {
    ...
    return (
        <button
            ...
            className={`px-4 py-1 rounded-full text-sm lg:text-base transition-colors ${isActive
                ? 'bg-primary-3'
                : 'border-2 border-primary-3'
                }`}
        >
            {label}
        </button>
    ...
    return (
        <div className='bg-neutral-5 border-b-2 border-b-primary-2 rounded-b-xl overflow-hidden'>
            ...
            <div className="p-4 space-y-3">
                <h3 className="font-semibold text-xs lg:text-lg">{course.title}</h3>
                ...
                <div className='space-y-2'>
                    <div className='font-semibold'>
                        {course.progress}% {isCompleted ? 'Selesai' : 'Progress belajarmu'}
                    </div>
                    <div className='w-full bg-zinc-300 rounded-full h-2'>
                        <div className={`h-2 rounded-full ${progressColor}`} style={{ width: `${course.progress}%` }} />
                    </div>
                </div>
                <button className={`w-full py-2 px-4 rounded-xl font-medium ${isCompleted
