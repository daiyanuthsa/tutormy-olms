                ...
                <div key={chapter.id} className="rounded-xl overflow-hidden">
                    <button
                        className={`w-full px-4 py-3 text-left font-semibold flex justify-between items-center 
                            ${isCurrentSection ? 'bg-primary-4' : 'bg-neutral-5'}`}
                    >
                        <span>{idx + 1}. {chapter.title}</span>
                        <Icon icon="tabler:chevron-down" className={`w-5 h-5 transition-transform ${isCurrentSection ? 'rotate-180' : ''}`} />
                    </button>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex w-full items-center gap-2 text-sm px-3 py-2 rounded-2xl 
                                            ${isActive ? 'bg-neutral-5 text-white' : 'text-white hover:bg-neutral-3'}`}
                                    >
                                        <Icon icon={lesson.is_locked ? "mdi:lock" : "mdi:play-circle"} className="w-5 h-5" />
                                        <div className='flex flex-col items-start'>
                                            <p>{lesson.title}</p>
                                            <p className="text-xs text-neutral-2">{lesson.duration}</p>
                                        </div>
                                    </button>
                                );
                            })}
        ...
        <div className="flex justify-between pt-3">
            <button
                ...
                className={`text-sm font-medium px-4 py-2 rounded-xl
                    ${isFirst ? 'bg-neutral-4 cursor-not-allowed' : 'border border-primary-2'}`}
            >
                ← Sebelum
            </button>
            <button
                ...
                disabled={isLast}
                className={`text-sm font-medium px-4 py-2 rounded-xl
                    ${isLast ? 'bg-neutral-4 cursor-not-allowed' : 'border border-primary-2'}`}
            >
                Sesudah →
            </button>
        </div>
